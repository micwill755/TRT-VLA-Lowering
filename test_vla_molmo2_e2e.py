...
def main():
    ...
    with torch.no_grad():
        ...
        encoder_kv_states = [
            (encoder_k[i], encoder_v[i]) for i in range(encoder_k.shape[0])
        ]
        ...
        step_modulation = action_expert.prepare_modulation_cache([step_timestep])[0]
        eager_hf_velocity = action_expert.forward_with_context(
            step_actions,
