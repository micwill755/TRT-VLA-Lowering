...
def convert_update_ssm_state(ctx: ConversionContext, target, args, kwargs, name):
    del target, kwargs
    args = list(args)
    hidden_states, ssm_a, ssm_b, ssm_c, ssm_d, dt, dt_bias, state, context_lengths = args[:9]
    dt_softplus, ngroups, nheads, head_dim, dstate = args[9:14]

    creator = get_trt_plugin_creator("update_ssm_state", "1", "")
    if creator is None:
        raise RuntimeError("update_ssm_state plugin not found in TensorRT plugin registry")
...
def convert_nvfp4_moe_plugin_geforce(ctx: ConversionContext, target, args, kwargs, name):
    del target, kwargs
