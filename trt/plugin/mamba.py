...
def _causal_conv1d_eager(
    ...
    return y.transpose(1, 2).to(dtype=hidden_states.dtype), new_state.to(dtype=conv_state.dtype)
...
def _update_ssm_state_eager(
    ...
    y = torch.zeros(batch, seq_len, nheads, head_dim, device=x.device, dtype=torch.float32)
    ...
    for bi in range(batch):
        ...
        for t in range(L):
            ...
            layer_state = layer_state * dA[:, None, None] + (
                dt_t[:, None] * x_t
            )[:, :, None] * b_h[:, None, :]
...
class PluginNemotronMamba(nn.Module):
    ...
    def forward(
        ...
        if getattr(self.norm, "forward", None) is not None and self.norm.__class__.__name__ == "MambaRMSNormGated":
