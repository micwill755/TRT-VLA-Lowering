...
from trt.executor.models.smolvla.helpers import make_smolvla_suffix_position_and_mask
from trt.executor.models.smolvla.load.serialize import SerializedSmolVLAAction
...
def preprocess(ctx: EdgeContext, inputs: dict) -> dict:
    ...
    if prefix_pad_mask is None:
        raise RuntimeError("SmolVLA diffusion requires prefix_pad_mask from the language stage")
    prefix_pad_mask = prefix_pad_mask.to(device=device)

    diffusion_module = StaticActionVelocityStepExportModule(
        step_encoder=SmolVLAPrefixKVStepEncoderExportModule(ctx.model),
        action_expert=SmolVLAExpertExportModule(ctx.model.vlm_with_expert),
...
def load(ctx: EdgeContext, inputs: dict) -> dict:
    serialized_action = SerializedSmolVLAAction(
